...
var defaultConfig = {
  apiKey: '47726291',
  apiSecret: '280ca507595a56c05fd9414809081fcbf9de44fd',
...
Client.prototype.patchBroadcast = function patchBroadcast(
  broadcastId,
  opts,
  cb
) {
  var url =
    this.c.apiUrl +
    this.c.endpoints.patchBroadcast
      .replace(/<%apiKey%>/g, this.c.apiKey)
...
Client.prototype.stopBroadcast = function (broadcastId, cb) {
  ...
  request.post(
    ...
    function (err, resp, body) {
      ...
      return cb(
        new Error('(' + resp.statusCode + ') ' + JSON.parse(body).message)
      );
    }
  );
};
...
Client.prototype.getBroadcast = function getBroadcast(broadcastId, cb) {
  ...
  request.get(
    ...
    function requestCallback(err, resp, body) {
      ...
      return cb(
        new Error('(' + resp.statusCode + ') ' + body ? body.message : '')
      );
    }
  );
};
...
Client.prototype.websocketConnect = function websocketConnect(opts, cb) {
  var url =
    this.c.apiUrl +
    this.c.endpoints.audioStreamer.replace(/<%apiKey%>/g, this.c.apiKey);
    ...
    function (err, resp, body) {
      ...
      if (resp.statusCode === 400) {
        if (body.code === 15204) {
          return cb(
            new Error(
              'SIP Interconnect for Video is not enabled in this project'
            )
          );
        }
        return cb(
          new Error(
            'Bad session ID, token, SIP credentials, or SIP URI (sip:user@domain.tld)'
          )
        );
      }
      ...
      if (resp.statusCode === 409) {
        return cb(
          new Error('Only Routed Sessions are allowed to initiate SIP Calls.')
        );
      }
